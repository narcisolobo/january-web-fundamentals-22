...
for (var i = 0; i < pokémon.length; i++) {
  if (pokémon[i].types.length == 1) {
    if (pokémon[i].types[0] == 'poison') {
      var pokémonName = pokémon[i].name;
      var pokémonNameToArray = pokémonName.split('');
      var pokémonNameToArrayReversed = pokémonNameToArray.reverse();
      var pokémonNameBackToString = pokémonNameToArrayReversed.join('');
      console.log(pokémonNameBackToString);
    }
  }
}